...
myLayout = avoidStruts
  $ smartBorders (layarPenuh ||| kiriKanan ||| atasBawah ||| ikan ||| siklik)
 where
...
dzenMenu :: [Char]
dzenMenu =
  "(zsh ~/.xmonad/menu.sh) | (dzen2 -w '30' -y '748' "
    ++ konfigurasiDzen
    ++ eksekusiMenu

eksekusiMenu :: [Char]
eksekusiMenu
  = " -m -p -l 7  -e 'button3=togglecollapse;leaveslave=collapse;button1=menuexec')"
...
dzenConky :: [Char]
dzenConky =
  "conky -c ~/.xmonad/conkyrc | dzen2 -x '1068' -y '748' -w '300' "
    ++ konfigurasiDzen

konfigurasiDzen :: [Char]
konfigurasiDzen =
  "-ta 'l' -h '20' -fg '#9f9fa2' -bg '#2b2b28' -fn '" ++ fontDzen ++ "'"
