...
myKeys conf@(XConfig {XMonad.modMask = mod}) = M.fromList $
    ...
    , ((mod .|. shiftMask       , xK_r          ), safePrompt "google-chrome-stable" myXPConfig)
    ...
    , ((mod .|. shiftMask       , xK_o          ), spawn "xfce4-screenshooter")
    ...
    , ((mod                     , xK_p          ), spawn "if type xmonad; then killall dzen2 && xmonad --recompile && xmonad --restart; else xmessage xmonad not in \\$PATH: \"$PATH\";fi")
